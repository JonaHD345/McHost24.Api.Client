...
namespace McHost24.Api.Client.Example
{
  ...
  internal sealed class ConsoleUi
  {
    ...
    /// <summary>
    /// Reads a password value while masking interactive console input.
    /// </summary>
    /// <param name="prompt">The prompt shown before reading the password.</param>
    /// <returns>The entered password.</returns>
    public string ReadPassword(string prompt)
    {
      Console.Write($"{prompt}: ");

      // Redirected input cannot be masked, so read it normally for scriptability.
      ...
      while (true)
      {
        // Handle keys manually so the password never gets echoed as plain text.
    ...
    /// <summary>
    /// Reads an optional integer value from the console.
    /// </summary>
    /// <param name="prompt">The prompt shown before reading the value.</param>
    /// <returns>The entered integer, or null when the input is empty.</returns>
    public int? ReadOptionalInt(string prompt)
    {
      while (true)
      {
        // Empty input is valid here, for example when the account has no 2FA code.
        Console.Write($"{prompt}: ");
        string? input = Console.ReadLine();
        ...
        WriteError("Enter a number or leave the value empty.");
      }
    }
    ...
    /// <returns>True when the user explicitly confirms with yes; otherwise false.</returns>
    public bool Confirm(string prompt)
    {
      // Default to "no" so potentially destructive actions need an explicit opt-in.
    ...
    /// <summary>
    /// Waits for the user before retrying the current workflow.
    /// </summary>
    public void WaitForRetry()
    {
      // Pause after recoverable failures so the user can see the reason.
      Console.WriteLine();
    ...
    {
      // Start from a clean console before login begins.
      ClearScreen();
      WriteAccent("MC-HOST24 API Console");
      Console.WriteLine("Login first, then choose any client action by number.");
      Console.WriteLine();
    }
    ...
    /// <summary>
    /// Writes warning-colored text.
    /// </summary>
    /// <param name="text">The text to write.</param>
    public void WriteWarning(string text)
    {
      // Use yellow for cancellations and non-fatal warnings.
    ...
    public void WriteApiException(McHost24ApiException exception)
    {
